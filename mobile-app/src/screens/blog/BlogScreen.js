  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]} edges={['top']}>
      ...
      {loading ? <ActivityIndicator size="large" color={c.primary} style={{ marginTop: 60 }} /> : loadError ? (
        ...
        <FlatList data={posts} keyExtractor={i => String(i.id)} contentContainerStyle={{ padding: spacing.base, paddingBottom: 120 }}
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => navigation.navigate('BlogDetail', { id: item.id, post: item })}
              style={[styles.card, { backgroundColor: c.card, borderColor: c.border }]}>
              {item.image && <RemoteImage source={item.image} width={500} style={styles.cardImg} fallback={<View style={[styles.cardImg, { backgroundColor: c.skeleton }]} />} />}
              <View style={styles.cardBody}>
                <Text numberOfLines={2} style={[styles.cardTitle, { color: c.text }]}>{getName(item, 'title')}</Text>
                <Text numberOfLines={2} style={[styles.cardExcerpt, { color: c.textSecondary }]}>{getName(item, 'content')?.replace(/<[^>]*>/g, '').slice(0, 120)}</Text>
                <Text style={[styles.cardDate, { color: c.textMuted }]}>{formatAppDate(item.createdAt, lang)}</Text>
              </View>
            </TouchableOpacity>
          )}
        />
