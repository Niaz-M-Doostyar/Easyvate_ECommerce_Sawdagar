      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={[styles.profileHero, { backgroundColor: c.secondary }]}>
          ...
          <View style={styles.heroHeader}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('Settings')} style={[styles.settingsAction, { backgroundColor: c.heroSurface }]}>
              <MaterialCommunityIcons name="cog-outline" size={20} color={c.heroText} />
            </TouchableOpacity>
          </View>
          <View style={styles.heroUserRow}>
            <View style={[styles.avatar, { backgroundColor: c.heroBorder }]}>
              <Text style={[styles.avatarText, { color: c.heroText }]}>{user.name?.charAt(0).toUpperCase()}</Text>
            </View>
            <View style={styles.heroUserCopy}>
              ...
              {user.role === 'supplier' && user.companyName ? (
                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6 }}>
                  <Text style={{ color: c.heroTextMuted, fontSize: fontSize.sm, fontWeight: fontWeight.semibold }}>{user.companyName}</Text>
...
function SummaryPill({ icon, label, value, c }) {
  return (
    <View style={[styles.summaryPill, { backgroundColor: c.heroSurface }]}>
      <MaterialCommunityIcons name={icon} size={18} color={c.heroTextMuted} />
      <Text numberOfLines={1} style={[styles.summaryLabel, { color: c.heroTextMuted }]}>{label}</Text>
      <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.75} style={[styles.summaryValue, { color: c.heroText }]}>{value}</Text>
    </View>
  );
}
  ...
  return (
    <View style={[styles.menuCard, { backgroundColor: c.card, borderColor: c.border }]}>
      {items.map((item, index) => (
        <TouchableOpacity key={item.label} onPress={item.action} style={[styles.menuItem, index < items.length - 1 && { borderBottomColor: c.border, borderBottomWidth: hairline }]}>
          <View style={[styles.menuIcon, { backgroundColor: c.brandSurface }]}>
            <MaterialCommunityIcons name={item.icon} size={19} color={c.primary} />
          </View>
          <Text numberOfLines={1} style={[styles.menuLabel, { color: c.text }]}>{item.label}</Text>
...
const styles = StyleSheet.create({
  ...
  settingsAction: { width: 44, height: 44, borderRadius: borderRadius.full, justifyContent: 'center', alignItems: 'center' },
  heroUserRow: { flexDirection: 'row', alignItems: 'center', marginTop: spacing.xl },
  heroUserCopy: { flex: 1, minWidth: 0, marginLeft: spacing.base },
  avatar: { width: 74, height: 74, borderRadius: borderRadius.full, justifyContent: 'center', alignItems: 'center' },
  ...
  summaryPill: { flex: 1, minWidth: 0, borderRadius: borderRadius.lg, padding: spacing.md },
  ...
  menuIcon: { width: 38, height: 38, borderRadius: borderRadius.full, justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  ...
  logoutBtn: { minHeight: 48, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingHorizontal: spacing.base, paddingVertical: spacing.md, borderRadius: borderRadius.full, borderWidth: 1 },
