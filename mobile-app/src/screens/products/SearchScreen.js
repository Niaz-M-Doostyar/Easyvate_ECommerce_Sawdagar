...
export default function SearchScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]} edges={['top']}>
      <ScreenHeader title={t.search} onBack={() => navigation.goBack()} />
      ...
      {loading ? (
          ...
          <View style={styles.suggestionWrap}>
            {quickSuggestions.map((item) => (
              <TouchableOpacity key={item} onPress={() => setQuery(item)} style={[styles.suggestionChip, { backgroundColor: c.brandSurface, borderColor: c.borderLight }]}> 
                ...
                <Text style={[styles.suggestionText, { color: c.primary }]}>{item}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <>
          ...
          <FlatList
            data={results} numColumns={2} keyExtractor={i => String(i.id)}
            contentContainerStyle={styles.grid}
            renderItem={({ item }) => (
              <View style={styles.gridItem}>
                <ProductCard product={item} onPress={() => { Keyboard.dismiss(); navigation.navigate('ProductDetail', { id: item.id, product: item }); }} />
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  searchRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, height: 48, borderRadius: borderRadius.xl, borderWidth: 1, gap: 8 },
  ...
  suggestionChip: { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: borderRadius.full, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 10 },
  suggestionText: { fontSize: fontSize.sm, fontWeight: fontWeight.bold },
  ...
  grid: { paddingHorizontal: spacing.base - 4, paddingTop: spacing.md },
